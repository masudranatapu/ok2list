...
use App\Mail\UserAdPromotionAdminGetMail;
use App\Mail\UserAdPromotionMail;
...
class PaymentController extends Controller
{
    ...
    public function offline_payment($payment_id)
    {
        ...
        if (Session::get('promid')) {
            ...
            // dd($packageExpired->format('d M Y'));
        }
            ...
            if (Session::get('promid')) {
                ...
                if (Auth::user()) {
                    ...
                    $details = [
                        ...
                        'body' => 'You just promote an ads a on ' . ' ' . $setting->website_title . ' .' . ' This promote ads will be expired on ' . $packageExpired->format('d M Y'),
                    ...
                    Mail::to($user->email)->send(new UserAdPromotionMail($details));
                }
                ...
                if ($admin) {
                    ...
                    $admindetails = [
                        ...
                        'body' => $user->name . ' was promote an ads on' . ' ' . $setting->website_title . '. ' . 'Please see what He/She promote on' . ' ' . $setting->website_title . '. ' . 'Ads expiry date is ' . $packageExpired->format('d M Y'),
                        'email' => 'His/Her email is : ' . $user->email,
                        'thanks' => 'Thank you for using ' . ' ' . $setting->website_title,
                        'site_url' => route('home'),
                        'site_name' => $setting->website_title,
                        'copyright' => Carbon::now()->format('Y') . ' ' . $setting->copyright . ' ' . $setting->website_title . ' ' . 'All rights reserved.',
                    ...
                    Mail::to($admin->email)->send(new UserAdPromotionAdminGetMail($admindetails));
                }
            }
            ...
            if (!empty($package)) {
                ...
                if (Auth::user()) {
                    ...
                    $details = [
                        'subject' => 'Message from ' . ' ' . $setting->website_title,
                        'greeting' => 'Hi ' . $user->name . ', ',
                        'body' => 'Congratulations you get a membership on' . ' ' . $setting->website_title . '. '. 'You package is'. ' '. $package->title . '. '.'This package will be expired on ' . $packageExpired->format('d M Y'),
                        'email' => 'Your email is : ' . $user->email,
                        'thanks' => 'Thank you for using ' . ' ' . $setting->website_title,
                        'site_url' => route('home'),
                        'site_name' => $setting->website_title,
                ...
                if ($admin) {
                    ...
                    $admindetails = [
                        'subject' => 'Message from ' . ' ' . $setting->website_title,
                        'greeting' => 'Hi ' . $admin->username . ', ',
                        'body' => $user->name.' '.'get premium membership on'.' ' .$setting->website_title. '. '.'His/Her package is'.' '.$package->title.'. '.'This package will be expired on'.' '.$packageExpired->format('d M Y'),
                        'email' => 'His/Her email is : ' . $user->email,
                        'thanks' => 'Thank you for using ' . ' ' . $setting->website_title,
                        'site_url' => route('home'),
                        'site_name' => $setting->website_title,
    ...
    public function sslcommerze()
    {
        ...
        $post_data['cancel_url'] = "https://www.Listorbuy.org/new_sslcz_gw/cancel.php";
