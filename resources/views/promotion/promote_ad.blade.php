                        ...
                        <div class="user-pro-section">
                            <div class="profile-details section">
                                ...
                                <div class="removepromo">
                                    <div class="promote">
                                        <p><img src="{{ asset('assets/images/promo.jpg') }}">Promote your ads to get more
                                            visibility.</p>
                                ...
                                <form action="javascript:void(0)" id="promotions">
                                    <div class="promo">
                                        <h4><img width="50" src="{{ $row->img_path_thumb }}" alt="Image"
                                                class="img-fluid"> {{ $row->ad_title }} </h4>
                                        <hr>
                                                        ...
                                                        <div class="card-header" id="headingOne">
                                                            <h2 class="mb-0 btn btn-block clktop">
                                                                <span data-toggle="collapse" data-target="#collapseOne"
                                                                    aria-expanded="true"
                                                                    aria-controls="collapseOne">{{ $promo->name }} <br>
                                                                    <span
                                                                        class="promo-hints">{{ $promo->description }}</span></span>
                                                            </h2>
                                                            <input type="hidden" value="{{ $promo->pk_no }}" name="promoId"
                                                                id="promoId">
                                                        </div>
                                                                ...
                                                                <table class="promotable">
                                                                    @if (isset($promo->allPromotions) && count($promo->allPromotions) > 0)
                                                                        @foreach ($promo->allPromotions as $ck => $cprom)
                                                                            ...
                                                                            @endphp
                                                                            <tr class="clktop7"
                                                                                for="cprom_{{ $cprom->pk_no }}">
                                                                                <td scope="col">
                                                                                    <input type="radio"
                                                                                        name="promotion_id"
                                                                                        value="{{ $cprom->pk_no }}"
                                                                                        id="cprom_{{ $cprom->pk_no }}"
                                                                                        data-price="{{ $discounted }}"
                                                                                        {{ $ck == 0 && $k == 0 ? 'checked' : '' }}>
                                                                                </td>
                                                                                <td scope="col">For
                                                                                    {{ $cprom->day_limit }} Days</td>
                                                                                <td scope="col"
                                                                                    style="text-align:center">
                                                                                    ₦{{ number_format($discounted, 2) }}
                                                                                    <sub><del>₦
                                                                                            {{ number_format($cprom->price, 2) }}</del></sub>
                                                                                    {{ Auth::user()->package->discount_on_promotion }}%
                                                                                    discount
                                                                                </td>
                                                                            </tr>
                                                                        @endforeach
                                        ...
                                        <div class="totalprice">
                                            <div class="section agreement">
                                                ...
                                                @endif
                                                <a href="{{ route('promotion.payment.gateway', ['price' => $discounted, 'adid' => $row->pk_no, 'promid' => 1]) }}"
                                                    class="btn btn-danger float-right" id="pay_btn"
                                                    onclick="return confirm('Are you sure !')">Pay Now</a>
