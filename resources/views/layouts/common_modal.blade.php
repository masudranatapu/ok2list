                            ...
                            @if (isset($data['category']) && count($data['category']) > 0)
                                @foreach ($data['category'] as $cat)
                                    <a class="nav-link {{ $category_query == $cat->url_slug ? 'active' : '' }}"
                                        id="v-pills-{{ $cat->url_slug }}-tab" data-toggle="pill"
                                        href="#{{ $cat->url_slug }}" role="tab"
                                        aria-controls="v-pills-{{ $cat->url_slug }}" aria-selected="true"><img
                                                ...
                                                @if (isset($data['subcategory']) && count($data['subcategory']) > 0)
                                                    @foreach ($data['subcategory'] as $scat)
                                                        @if ($cat->pk_no == $scat->parent_id)
                                                            <li><a
                                                                    href="{{ route('ads.list', ['area' => $area_query, 'category' => $scat->url_slug]) }}">{{ $scat->name }}</a>
                                                            </li>
                                                        @endif
                                                    @endforeach
                                                @endif
                            ...
                            @if (isset($data['cities']) && count($data['cities']) > 0)
                                @foreach ($data['cities'] as $key => $city)
                                    <a class="nav-link {{ $data['area_query'] == $city->url_slug ? 'active' : '' }}"
                                        id="v-pills-{{ $city->url_slug }}-tab" data-toggle="pill"
                                        href="#{{ $city->url_slug }}" role="tab"
                                        aria-controls="v-pills-{{ $city->url_slug }}"
                                        aria-selected="true">{{ $city->name ?? '' }}<i
                            ...
                            <div class="nav flex-column nav-pills" id="v-pills-tab-division" role="tablist"
                                ...
                                @foreach ($data['divisions'] as $key => $divi)
                                    <a class="nav-link {{ $data['area_query'] == $divi->url_slug ? 'active' : '' }}"
                                        id="v-pills-{{ $divi->url_slug }}-tab" data-toggle="pill"
                                        href="#{{ $divi->url_slug }}" role="tab"
                                        aria-controls="v-pills-{{ $divi->url_slug }}"
                                        aria-selected="true">{{ $divi->name }}<i
                                            class="fa fa-angle-right float-right"></i></a>
                                @endforeach
                                                ...
                                                @if (isset($data['areas']) && count($data['areas']) > 0)
                                                    @foreach ($data['areas'] as $ak => $area)
                                                        @if ($area->city_pk_no == $city->pk_no)
                                                            <li><a
                                                                    href="{{ route('ads.list', ['area' => $area->url_slug, 'category' => $get_category_query]) }}">{{ $area->name }}</a>
                                                            </li>
                                                        @endif
                                                    @endforeach
                                                @endif
                            ...
                            @if (isset($data['divisions']) && count($data['divisions']) > 0)
                                @foreach ($data['divisions'] as $key => $divi)
                                    <div class="tab-pane fade" id="{{ $divi->url_slug }}" role="tabpanel"
                                        ...
                                        <div class="tabs-cat-list">
                                            <h3><a href="{{ $divi->url_slug }}"><i
                                                        class="fa fa-map-marker-alt"></i>{{ $divi->name }}</a></h3>
                                            <ul>
                                                @if (isset($data['areas']) && count($data['areas']) > 0)
                                                    @foreach ($data['areas'] as $ak => $area)
                                                        @if ($area->division_pk_no == $divi->pk_no)
                                                            <li><a
                                                                    href="{{ route('ads.list', ['area' => $divi->url_slug]) }}">{{ $area->name }}</a>
                                                            </li>
                                                        @endif
                                                    @endforeach
                                                @endif
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <a href="{{ route('ad-post', ['type' => 'jobs']) }}">
                                <div class="item">
                                    <img src="{{ asset('assets/images/icon/7.png') }}" alt="images"
                                        class="img-fluid">
                                    <p>{{ __('post_job') }}</p>
                                </div>
                            </a>
                        ...
                        <div class="col-sm-12">
                            <a href="{{ route('ad-post', ['type' => 'property']) }}">
                                <div class="item">
                                    <img src="{{ asset('assets/images/icon/3.png') }}" alt="images"
                                        class="img-fluid">
                                    <p>{{ __('rent') }}</p>
                                </div>
                            </a>
