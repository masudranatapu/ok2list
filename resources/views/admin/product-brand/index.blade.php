...
@section('title') {{ __('product_brand') }} @endsection
@section('page-name') {{ __('product_brand') }} @endsection

@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="{{ route('admin.dashboard') }}">{{ __('admin_breadcrumb_title') }}</a>
    </li>
    <li class="breadcrumb-item active">{{ __('product_brand') }}</li>
@endsection
                            ...
                            <div class="form-group">
                                @if (hasAccessAbility('new_brand', $roles))
                                    <a class="text-white btn btn-round btn-sm btn-primary"
                                        href="{{ route('admin.brand.create') }}"><i class="ft-plus text-white"></i>
                                        {{ __('create_bra') }}
                                    </a>
                                @endif
                            </div>
                                            ...
                                            @foreach ($rows as $row)
                                                <tr>
                                                    ...
                                                    <td class="text-left" style="width: 20%;">
                                                        {{ $row->category->name ?? '' }}</td>
                                                    <td class="text-left" style="width: 20%;">
                                                        {{ $row->subcategory->name ?? '' }}</td>
                                                    <td class="text-left" style="width: 20%;">{{ $row->name }}</td>
                                                    <td style="width: 10%;">
                                                        @if (hasAccessAbility('edit_brand', $roles))
                                                            <a href="{{ route('admin.brand.edit', [$row->pk_no]) }}"
                                                                class="btn btn-xs btn-outline-primary mr-0 mb-1"><i
                                                                    class="la la-edit"></i></a>
                                                        @endif
                                                        @if (hasAccessAbility('delete_brand', $roles))
                                                            <a href="{{ route('admin.brand.delete', [$row->pk_no]) }}"
                                                                onclick="return confirm('Are you sure you want to delete?')"
                                                                class="btn btn-xs btn-outline-danger mr-0 mb-1"><i
