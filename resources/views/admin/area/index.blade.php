                        ...
                        <div class="card-header pl-2">
                            @if (hasAccessAbility('new_area', $roles))
                                <a class="text-white btn btn-round btn-sm btn-primary"
                                    href="{{ route('admin.area.create') }}"><i class="ft-plus text-white"></i>Add Area</a>
                            @endif
                            &nbsp;&nbsp;
                            <select class="form-control" style="width: 150px; display: inline;" id="filterArea">
                                ...
                                <option value="{{ route('admin.area.list') }}?type=division"
                                    {{ $type == 'division' ? 'selected' : '' }}> Division Areas </option>
                                <option value="{{ route('admin.area.list') }}?type=city"
                                    {{ $type == 'city' ? 'selected' : '' }}> City Areas </option>
                            </select>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered alt-pagination table-sm"
                                        ...
                                        <thead>
                                            <tr>
                                                <th>Sl.</th>
                                                <th class="text-left">{{ __('area') }}</th>
                                                <th class="text-left">{{ __('city_division') }}</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if (hasAccessAbility('edit_area', $roles))
                                                            <a href="{{ route('admin.area.edit', [$row->pk_no]) }}"
                                                                title="EDIT"
                                                                class="btn btn-xs btn-outline-primary mr-1"><i
                                                                    class="la la-edit"></i></a>
                                                        @endif
                                                        @if (hasAccessAbility('delete_area', $roles))
                                                            <a href="{{ route('admin.area.delete', [$row->pk_no]) }}"
                                                                onclick="return confirm('Are you sure you want to delete  area ?')"
