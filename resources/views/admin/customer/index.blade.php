                ...
                <div class="col-md-12">
                    <div class="card card-sm">
                        <div class="card-header">
                            <a class="btn btn-round btn-sm btn-primary text-white" href="#"
                                title="CUSTOMER LIST">{{ __('customer_list') }}</a>
                            ...
                            <div class="card-body card-dashboard">
                                <div class="table-responsive ">
                                    <table class="table table-striped table-bordered alt-pagination table-sm"
                                        ...
                                        <tbody>
                                            @foreach ($rows as $row)
                                                <tr class="{{ $row->is_active == 2 ? 'text-warning' : '' }}"
                                                    title="{{ $row->is_active == 2 ? 'Inactive user' : '' }}">
                                                    <td class="text-center">{{ $loop->index + 1 }}</td>
                                                    <td>
                                                        <span class="text-upper">{{ $row->name }}</span>
                                                    </td>
                                                    <td>{{ $row->email }}</td>
                                                    <td>
                                                        <p>{{ $row->mobile1 }}</p>
                                                    ...
                                                    <td>
                                                        @if ($row->package)
                                                            ...
                                                            @if ($row->package->pk_no > 1)
                                                                <div>Start :
                                                                    {{ date('d-m-y', strtotime($row->package_start_date)) }}
                                                                </div>
                                                                <div>End :
                                                                    {{ date('d-m-y', strtotime($row->package_end_date)) }}
                                                                </div>
                                                            @endif
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('admin.customer.ads', $row->id) }}"
                                                            class="btn btn-xs btn-outline-warning mr-1"
                                                            title="View Customner Ads">
                                                            View Ads
                                                        </a>
                                                    </td>
                                                    ...
                                                    @endif --}}
                                                            ...
                                                            <a href="{{ route('admin.customer.delete', [$row->id]) }}"
                                                                ...
                                                                title="INACTIVE"><i class="la la-trash"></i></a>
