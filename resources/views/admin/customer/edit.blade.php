...
@section('title') {{ __('edit_customer') }} @endsection
@section('page-name') {{ __('edit_customer') }} @endsection
                    ...
                    <div class="card card-sm">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                {!! Form::open([
                                    'route' => ['admin.customer.update', $row->id],
                                    'method' => 'post',
                                ...
                                <h5><label>{{ __('email') }}: <strong>{{ $row->email }}</strong></label></h5>
                                <div class="row">
                                    <div class="col-md-4">
                                                        ...
                                                        <optgroup label="City">
                                                            @foreach ($city_combo as $key => $city)
                                                                <option value="{{ $city->pk_no }}" data-type="city"
                                                                    data-url="{{ route('get-area-by-location', ['city', $city->pk_no]) }}">
                                                                    {{ $city->name }}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endif
                                                    @if ($division_combo)
                                                        <optgroup label="Division">
                                                            @foreach ($division_combo as $key => $divis)
                                                                <option value="{{ $divis->pk_no }}" data-type="division"
                                                                    data-url="{{ route('get-area-by-location', ['division', $city->pk_no]) }}">
                                                                    {{ $divis->name }}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endif
                                        ...
                                        <div class="form-group {!! $errors->has('is_verified') ? 'error' : '' !!}">
                                            ...
                                            <div class="controls">
                                                <select class="form-control mb-1" name="is_verified">
                                                    <option value="1"
                                                        @php if ($row->is_verified == 1) { echo "selected"; } @endphp>Yes
                                                    </option>
                                                    <option value="0"
                                                        @php if ($row->is_verified == 0) { echo "selected"; } @endphp>No
                                                    </option>
                                                </select>
                                        ...
                                        <div class="form-group {!! $errors->has('is_active') ? 'error' : '' !!}">
                                            ...
                                            <div class="controls">
                                                <select class="form-control mb-1" name="is_active">
                                                    <option value="1"
                                                        @php if ($row->is_active == 1) { echo "selected"; } @endphp>Yes
                                                    </option>
                                                    <option value="0"
                                                        @php if ($row->is_active == 0) { echo "selected"; } @endphp>No
                                                    </option>
                                                </select>
                                ...
                                <div class="col-md-4 offset-md-4">
                                    ...
                                    <button type="submit" class="btn btn-primary">
                                        <i class="la la-check-square-o"></i>{{ __('update_basic_info') }}
                                    </button>
                                    <br>
                                </div>
